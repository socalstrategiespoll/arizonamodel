...
TAU2_CAP = 0.05import numpy as np
...
_CONFIG = {
    "Maricopa":   dict(total=424302, early_share=0.677, early_bso=(0.720, 0.246, 0.034), dayof_bso=(0.753, 0.213, 0.034), late_share=0.1454, late_bso=(0.753, 0.213, 0.034)),
    "Pima":       dict(total=82973,  early_share=0.644, early_bso=(0.707, 0.234, 0.059), dayof_bso=(0.738, 0.202, 0.059)),
    "Yavapai":    dict(total=50489,  early_share=0.678, early_bso=(0.727, 0.235, 0.038), dayof_bso=(0.760, 0.203, 0.038)),
    "Pinal":      dict(total=42007,  early_share=0.736, early_bso=(0.728, 0.234, 0.039), dayof_bso=(0.761, 0.201, 0.038)),
    "Mohave":     dict(total=33460,  early_share=0.753, early_bso=(0.730, 0.225, 0.045), dayof_bso=(0.762, 0.193, 0.045)),
    "Cochise":    dict(total=14913,  early_share=0.629, early_bso=(0.687, 0.249, 0.064), dayof_bso=(0.718, 0.218, 0.064)),
    "Yuma":       dict(total=12332,  early_share=0.835, early_bso=(0.712, 0.229, 0.059), dayof_bso=(0.744, 0.197, 0.059)),
    "Navajo":     dict(total=12104,  early_share=0.575, early_bso=(0.720, 0.237, 0.043), dayof_bso=(0.752, 0.205, 0.043)),
    "Coconino":   dict(total=10195,  early_share=0.612, early_bso=(0.718, 0.235, 0.047), dayof_bso=(0.751, 0.203, 0.046)),
    "Gila":       dict(total=9219,   early_share=0.728, early_bso=(0.757, 0.205, 0.038), dayof_bso=(0.790, 0.172, 0.038)),
    "Apache":     dict(total=4512,   early_share=0.512, early_bso=(0.713, 0.209, 0.078), dayof_bso=(0.745, 0.178, 0.077)),
    "Graham":     dict(total=4046,   early_share=0.664, early_bso=(0.725, 0.231, 0.044), dayof_bso=(0.757, 0.198, 0.045)),
    "La Paz":     dict(total=2093,   early_share=0.546, early_bso=(0.728, 0.211, 0.061), dayof_bso=(0.760, 0.178, 0.062)),
    "Santa Cruz": dict(total=1725,   early_share=0.695, early_bso=(0.717, 0.218, 0.065), dayof_bso=(0.749, 0.187, 0.065)),
    "Greenlee":   dict(total=618,    early_share=0.515, early_bso=(0.751, 0.185, 0.065), dayof_bso=(0.785, 0.150, 0.064)),
}
...
class County:
    def __init__(self, name, total, early_share, early_bso, dayof_bso, late_share=0.0, late_bso=None):
        self.name = name
        self.region = COUNTY_REGION[name]
        self.total = total
        self.early_total_proj = total * early_share
        self.late_total_proj = total * late_share
        self.dayof_total_proj = total * (1 - early_share - late_share)
        self.prior_early_bso = early_bso
        self.prior_dayof_bso = dayof_bso
        self.prior_late_bso = late_bso if late_bso is not None else dayof_bso
        self.reported_early = None
        self.reported_dayof = None
        self.reported_late = None

    def prior_bso(self, bucket):
        if bucket == "early":
            return self.prior_early_bso
        if bucket == "late":
            return self.prior_late_bso
        return self.prior_dayof_bso

    def reported(self, bucket):
        if bucket == "early":
            return self.reported_early
        if bucket == "late":
            return self.reported_late
        return self.reported_dayof

    def report(self, bucket, b, s, o):
        if bucket == "early":
            self.reported_early = (b, s, o)
        elif bucket == "late":
            self.reported_late = (b, s, o)
        else:
            self.reported_dayof = (b, s, o)

    def projected_total(self, bucket):
        if bucket == "early":
            return self.early_total_proj
        if bucket == "late":
            return self.late_total_proj
        return self.dayof_total_proj
...
def get_amplified_factor(bucket, quantity):
    ...
    direct = hierarchical_pool(obs, bucket, *floors)
    ...
    state_mean, state_var = blend(inf_state_mean, inf_state_var,
                                   direct["state_mean"], direct["state_var"])
    region_means, region_vars = {}, {}
    for g in REGIONS:
        region_means[g], region_vars[g] = blend(
            inf_region_means[g], inf_region_vars[g],
            direct["region_means"][g], direct["region_vars"][g])

    county_tau2 = max(direct["county_tau2"], floors[2] ** 2)
    ...
    return dict(state_mean=state_mean, state_var=max(state_var, floors[0] ** 2),
                ...
                county_tau2=county_tau2, n_reporting=direct["n_reporting"])


def get_dayof_factor(quantity):
    return get_amplified_factor("dayof", quantity)


def get_late_factor(quantity):
    return get_amplified_factor("late", quantity)
    ...
    for bucket in ("early", "dayof", "late"):
        for quantity in ("B", "S", "T"):
            if bucket == "early":
                ...
                factors[(bucket, quantity)] = get_dayof_factor(quantity)
            else:
                factors[(bucket, quantity)] = get_late_factor(quantity)
...
def county_point_estimate_remaining(name):
    ...
    for bucket, factors in (("early", early_f), ("dayof", dayof_f), ("late", late_f)):
        if county.projected_total(bucket) <= 0:
            continue
...
def snapshot(n_sims=N_SIMS):
    ...
    for name, county in COUNTIES.items():
        ...
        late = county.reported("late") or (0, 0, 0)
        reported = {
            "B": early[0] + dayof[0] + late[0],
            "S": early[1] + dayof[1] + late[1],
            "O": early[2] + dayof[2] + late[2],
        }
...
_CONFIG = {
    "Maricopa":   dict(total=424302, early_share=0.677, early_bso=(0.720, 0.246, 0.034), dayof_bso=(0.753, 0.213, 0.034)),
    "Pima":       dict(total=82973,  early_share=0.644, early_bso=(0.707, 0.234, 0.059), dayof_bso=(0.738, 0.202, 0.059)),
    "Yavapai":    dict(total=50489,  early_share=0.678, early_bso=(0.727, 0.235, 0.038), dayof_bso=(0.760, 0.203, 0.038)),
    "Pinal":      dict(total=42007,  early_share=0.736, early_bso=(0.728, 0.234, 0.039), dayof_bso=(0.761, 0.201, 0.038)),
    "Mohave":     dict(total=33460,  early_share=0.753, early_bso=(0.730, 0.225, 0.045), dayof_bso=(0.762, 0.193, 0.045)),
